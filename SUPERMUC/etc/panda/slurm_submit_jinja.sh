...
#SBATCH -J w_{{workerID}}
#Output and error (also --output, --error):
#SBATCH -o {{workspec.accessPoint}}/slurm_stdout.txt
#SBATCH -e {{workspec.accessPoint}}/slurm_stderr.txt
...
#SBATCH --export=NONE
#SBATCH --get-user-env
#SBATCH --account="pr58be"
#SBATCH --mcs-label=pr58be
# Wall clock limit:
#SBATCH --time=01:20:00
# maxWalltime {{workspec.maxWalltime}} {{workspec.maxWalltime//60}}
#SBATCH --time-min=75
#SBATCH --partition={% if workspec.jobspec_list|length<=16 %}"micro"{% else %}"general" {% endif %}

#SBATCH --ntasks-per-node 1
#SBATCH -N {{workspec.jobspec_list|length}}

set -e 

POSTMORTEM_FILE={{workspec.accessPoint}}/FINISHED
...
# Hack to force broken PRE pilot to source /release_setup.sh
export ALRB_CONT_SETUPFILE=dummy
...
export HARVESTER_WORKER_ID={{workspec.workerID}}
export HARVESTER_ACCESS_POINT={{workspec.accessPoint}}
export HARVESTER_NNODE={{workspec.jobspec_list|length}}
export HARVESTER_NTASKS=$nTasks
export HARVESTER_CONTAINER_IMAGE="atlas-grid-centos7:latest"
...
for PANDA_JOB_ID in $PANDA_JOB_IDS;  do
   ...
   # export IMAGE_BASE=/hppfs/scratch/01/ri32buz3/images/
...
# Write a status file so squeue monitor can see worker if finshed
# curly brackets looks like template
#touch $HARVESTER_ACCESS_POINT/FINISHED
