...
while [ $i -lt 10 ]; do
    ...
    if [ -z "$rc" ]; then
        ...
        if [ "/tmp/harvester.db" -nt "/home/dbenjamin/harvester-py3/var/lib/harvester.db" ] ; then
            echo "/tmp/harvester.db is newer than /home/dbenjamin/harvester-py3/var/lib/harvester.db"
	        cp -fv /tmp/harvester.db /home/dbenjamin/harvester-py3/var/lib/harvester.db                                        
		fi
	rm -v $pidfile
        exit 0
    fi
