...
while [ $i -lt 10 ]; do
    ...
    if [ -z "$rc" ]; then
        ...
        if [ "/tmp/harvester.db" -nt "/gpfs/alpine/hep113/world-shared/harvester/var/lib/harvester.db" ] ; then
            echo "/tmp/harvester.db is newer than /gpfs/alpine/hep113/world-shared/harvester/var/lib/harvester.db"
	    cp -fv /tmp/harvester.db /gpfs/alpine/hep113/world-shared/harvester/var/lib/harvester.db                                        
	fi
	rm -v $pidfile
        exit 0
    fi
