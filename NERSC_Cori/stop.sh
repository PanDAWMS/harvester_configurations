...
while [ $i -lt 10 ]; do
    ...
    if [ -z "$rc" ]; then
        ...
        if [ "/tmp/harvester.db" -nt "/global/cfs/cdirs/m2616/harvester/var/lib/harvester.db" ] ; then
            echo "/tmp/harvester.db is newer than /global/cfs/cdirs/m2616/harvester/var/lib/harvester.db"
	    cp -fv /tmp/harvester.db /global/cfs/cdirs/m2616/harvester/var/lib/harvester.db                                        
	fi
	rm -v $pidfile
        exit 0
    fi
